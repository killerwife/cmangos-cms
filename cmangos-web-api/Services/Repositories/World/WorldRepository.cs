    ...
    public class WorldRepository : IWorldRepository
    {
        ...
        public async Task<(List<CreatureWithSpawnGroup>, float, float, float, float)?> GetCreaturesForZoneAndEntry(int mapId, int zoneId, uint entry, int index)
        {
            ...
            var gosWithSpawnGroup = await _context.Database.SqlQuery<CreatureWithSpawnGroup>(
                    $"SELECT creature.*, spawn_group.Id AS spawn_group_id, spawn_group_formation.PathId as formation_path_id FROM creature LEFT JOIN spawn_group_spawn ON creature.guid=spawn_group_spawn.Guid LEFT JOIN spawn_group ON (spawn_group.Id=spawn_group_spawn.Id AND spawn_group.type=0) LEFT JOIN spawn_group_entry ON spawn_group_entry.Id=spawn_group.Id LEFT JOIN creature_spawn_entry ON creature.guid=creature_spawn_entry.guid LEFT JOIN spawn_group_formation ON spawn_group.Id=spawn_group_formation.Id WHERE (creature.id={entry} OR spawn_group_entry.entry={entry} OR creature_spawn_entry.entry={entry}) AND creature.map={mapId} AND (spawn_group.type IS null OR spawn_group.type=0) AND (spawn_group.Id IS NULL OR spawn_group.Id IS NOT NULL) AND position_x > {minZoneX} && position_x < {maxZoneX} && position_y > {minZoneY} && position_y < {maxZoneY}").ToListAsync();
            return (gosWithSpawnGroup, (float)minZoneY, (float)maxZoneY, (float)minZoneX, (float)maxZoneX);
        }
